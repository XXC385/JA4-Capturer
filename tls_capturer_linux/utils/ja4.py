...
def printout(x, ja_type):
    ...
    if 'ja4x' in output_types:  # ja_type == 'JA4X':
        ...
        if not debug:
            delete_keys(['cert_extensions', 'extension_lengths', 'issuers', 'subjects', 'rdn_oids', 'issuer_sequence',
                         'subject_sequence', 'issuer_hashes', 'subject_hashes'], final)
...
def layer_update(x, pkt, layer):
    ...
    if layer == 'quic':
        ...
        if quic:
            if isinstance(quic, list):
                quic = quic[0]
            [x.update({key: quic[f'{layer}_{layer}_{item}']}) for key, item in keymap[layer].items() if
             f'{layer}_{layer}_{item}' in quic]
    ...
    if l:
        [x.update({key: l[f'{layer}_{layer}_{item}']}) for key, item in keymap[layer].items() if
         f'{layer}_{layer}_{item}' in l]
    ...
    if layer == 'x509af' and l:
        ...
        x.update({'printable_certs': l[
            'x509sat_x509sat_printableString']}) if 'x509sat_x509sat_printableString' in l else None
...
def main():
    ...
    parser.add_argument("--ja4l", "--ja4l", action="store_true", default=False,
    ...
    parser.add_argument("--ja4ssh", "--ja4ssh", action="store_true", default=False,
    ...
    parser.add_argument("-o", "--original_rendering", required=False, action="store_true",
    ...
    if args.key:
        ps = Popen(["tshark", "-r", args.pcap, "-o", f"tls.keylog_file:{os.path.abspath(args.key)}", "-T", "ek", "-n"],
    ...
    for idx, line in enumerate(iter(ps.stdout.readline, '')):  # enumerate(sys.stdin):
        if "layers" in line:
            ...
            [cache_update(x, key, x[key], STREAM) for key in
             ['stream', 'src', 'dst', 'srcport', 'dstport', 'protos']]  # if x['srcport'] != '443' else None
            ...
            if 'tcp' in x['protos']:  # and 'tls' not in x['protos']:
                if 'flags' in x:
                    ...
                    if (flags & TCP_FLAGS['ACK']) and not (flags & TCP_FLAGS['SYN']) and 'ack' in x and x[
                        'ack'] == '1' and 'seq' in x and x['seq'] == '1':
